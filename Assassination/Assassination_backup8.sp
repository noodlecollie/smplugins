...
#define PLUGIN_VERSION		"0.1.1.41"	// Note: I don't update this religiously on builds. :P There have been AT LEAST this many builds.
...
// #TODO#: Put PLAYER-THRESHOLD into the code where it should be.
...
new bool:b_SpawnEdit = false;	// If this is true, we're in spawn edit mode. If STATE_PLAYERS_ROUND_RESTARTING is also present, don't do anything.
new n_SpawnEditClient = -1;		// Records which client is editing.
...
public OnPluginStart()
{
	...
	RegAdminCmd("nfas_spawn_edit",	AdminCommand_SpawnEdit, ADMFLAG_CONVARS, "Creates spawn point files for maps.");
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	#endif
	if ( (g_PluginState & STATE_NO_ACTIVITY) == STATE_NO_ACTIVITY && !b_SpawnEdit ) return;
...
public Event_TeamsChange(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ( PLAYER_THRESHOLD < 1 )
	{
		LogMessage("FATAL ERROR: Player threshold %d is < 1! TeamChange logic cannot continue.", PLAYER_THRESHOLD);
		return;
	}
	
	new tc_ClientIndex = GetClientOfUserId(GetEventInt(event, "userid"));
	new tc_OldTeam = GetEventInt(event, "oldteam");	// These will be TEAM_RED, TEAM_BLUE, etc.
	new tc_NewTeam = GetEventInt(event, "team");
	new tcb_Disconnect = GetEventBool(event, "disconnect");
	new tc_OldTeamCount = -1;
	new tc_OldTeamCount_post = -1;
	new tc_NewTeamCount= -1;
	new tc_NewTeamCount_post = -1;
	
	// If the client's old or new team is less than TEAM_RED, we don't need to worry about calculating the team counts.
	// These counts will remain as -1.
	// If an old team count is -1, this means the client is joining the match in progress.
	// If a new team count is -1, this means the client is leaving the match in progress.
	
	if ( tc_OldTeam >= TEAM_RED )
	{
		tc_OldTeamCount = GetTeamClientCount(tc_OldTeam);
		tc_OldTeamCount_post = (tc_OldTeamCount - 1);
	}
	
	if ( tc_NewTeam >= TEAM_RED )
	{
		tc_NewTeamCount = GetTeamClientCount(tc_NewTeam);
		tc_NewTeamCount_post = (tc_NewTeamCount + 1);
	}
	
	// The client's old team is currently above the threshold
	if ( tc_OldTeamCount >= PLAYER_THRESHOLD )
	{
		// The client's old team will still be above the threshold after the team change.
		if ( tc_OldTeamCount_post >= PLAYER_THRESHOLD )
		{
			// The client's new team is currently above the threshold.
			if ( tc_NewTeamCount >= PLAYER_THRESHOLD )
			{
				// #TODO#: Stay enabled.
			}
			// The client's new team is not currently above the threshold.
			else
			{
				// #TODO#: Check if the count is -1.
				
				// The client's new team will be above the threshold after the team change.
				if ( tc_NewClientCount_post >= PLAYER_THRESHOLD )
				{
					// #TODO#: Enable the plugin.
				}
				// The client's new team won't be above the threshold after the team change.
				else
				{
					// #TODO#: Stay disabled.
				}
		...
		// The client's old team will be below the threshold after the team change.
		else
		{
			// #TODO#: Disable the plugin.
		}
	}
	// The client's old team is currently below the player threshold
	else
	{
		// #TODO#: Check if the count is -1.
		// #TODO#: Stay disabled.
...
/*	Manages creating custom spawn points for a map.	*/
public Action:AdminCommand_SpawnEdit(client, args)
{
	if ( n_SpawnEditClient > -1 && n_SpawnEditClient != client )
	{
		decl String:PlayerName[MAX_NAME_LENGTH + 1];
		GetClientName(n_SpawnEditClient, PlayerName, sizeof(PlayerName));
		
		ShowActivity2(client, "[AS]", "%s %t", PlayerName, "as_player_already_editing");
	...
	if ( b_SpawnEdit )
	{
		ShowActivity2(client, "[AS]", "%t", "as_leaving_spawn_edit_mode");
		b_SpawnEdit = false;
		n_SpawnEditClient = -1;
		...
		LogMessage("[AS] Leaving spawn edit mode. Restarting map (%s)...", mapname);
	...
	{
		ShowActivity2(client, "[AS]", "%t", "as_entering_spawn_edit_mode");
		
		// Set the STATE_NO_ACTIVITY flag and restart the round.
		// This will ensure no other features activate until the map is reset.
		...
		b_SpawnEdit = true;			// Set the spawn edit flag.
		n_SpawnEditClient = client;	// Note which client is editing.
