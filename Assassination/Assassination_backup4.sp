...
code itself, please forgive me. It's an idiosyncracy that is very hard for me to break.
:P
...
public Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	n_TargetIndex = RandomPlayerFromTeam(TEAM_BLUE);
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	// If the player count is too low then the assassin and victim indices will be 0 anyway, so
	// the player clients won't match and we don't need to worry about exiting out.
	...
	if ( pd_ClientIndex = n_AssassinIndex )
	{
		...
		// If the killer wasn't a player, or was a player from the same team:
		else if ( !IsClientConnected(pd_AttackerIndex) || pd_AttackerIndex > MaxClients || pd_AttackerIndex < 1 || GetClientTeam(pd_ClientIndex) == GetClientTeam(pd_AttackerIndex) )
		{
			// Clear both indices.
			n_AssassinIndex = 0;
			n_TargetIndex = 0;
	...
	{
		// If the killer wasn't a player:
		if ( !IsClientConnected(pd_AttackerIndex) || pd_AttackerIndex > MaxClients || pd_AttackerIndex < 1 )
		{
			// Clear the target index.
			n_TargetIndex = 0;
			
			// Choose another random player from the same team, excluding the late target.
			n_TargetIndex = RandomPlayerFromTeam(, pd_ClientIndex
		}
	}
}
