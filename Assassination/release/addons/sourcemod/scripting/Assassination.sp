...
/*
	Recent changes:
	- Fixed localisation; this was unnoticeable since all the plugin text was in English, but if a foreign player had played with a different lauguage set once the plugin had been translated, there would have been errors.
	- Changed the assassin and target sprites to be particle effects instead. This should prevent them from being seen by the player they're parented to.
	- Changed the method of winning the round for a team to not include toggling sv_cheats, for neatness purposes.
	- Added the admin command "nfas_switch" to allow an admin to assign the assassin or target.
	- Fixed the lack of music when the target was killed - the sound was not added to the download table properly.
...
#include <keyvalues>
#define DEBUG	0
// #TODO#: Remove the DEBUG flag from nfas_switch and its global variables
...
#define PLUGIN_VERSION		"1.0.0.44"
...
#define ASSASSIN_PARTICLE		"assassin_red"
#define TARGET_PARTICLE			"target_red"
...
public Plugin:myinfo = 
{
	...
	url = "http://forums.alliedmods.net/showthread.php?p=1531506#post1531506"
}
...
public OnPluginStart()
{
	...
	// Admin commands:
	#if DEBUG == 1
	RegAdminCmd("nfas_switch",		AdminCommand_Switch, ADMFLAG_SLAY, "Allows the assassin or target player to be changed.");
	#endif
	...
	if ( IsServerProcessing() )
	{
		...
		PrintToChatAll("[AS] %T", "as_pluginloadnextmapchange", LANG_SERVER);
...
public OnMapStart()
{
	...
	*/
	...
	AddFileToDownloadsTable(PARTICLE_PCF);
	PrecacheGeneric(PARTICLE_PCF, true);
...
public OnGameFrame()
{
	// Icons will now be managed entirely from OnGameFrame.
	// Note that these are now particle effects, I just CBA to rename all the variables.
	...
	if ( g_PluginState < STATE_DISABLED )
	{
		...
		if ( AssassinSprite[0] > MaxClients && IsValidEntity(AssassinSprite[0]) )
		{
			...
			{
				#if DEBUG == 1
				LogMessage("Killing assassin icon...");
				#endif
				
				AcceptEntityInput(AssassinSprite[0], "Stop");
				AcceptEntityInput(AssassinSprite[0], "Kill");
				AssassinSprite[0] = -1;
				AssassinSprite[1] = -1;
			}
			// NO LONGER NEEDED: If the assassin is valid and the same as the assigned player, handle movement.
		...
		else if ( AssassinSprite[0] < 0 )
		{
			...
			{
				#if DEBUG == 1
				LogMessage("Creating assassin icon...");
				#endif
				
				AssassinSprite[0] = AttachParticle(GlobalIndex(false, 0), ASSASSIN_PARTICLE, 32.0);
		...
		if ( TargetSprite[0] > MaxClients && IsValidEntity(TargetSprite[0]) )
		{
			...
			{
				#if DEBUG == 1
				LogMessage("Killing target icon...");
				#endif
				
				AcceptEntityInput(TargetSprite[0], "Stop");
				AcceptEntityInput(TargetSprite[0], "Kill");
				TargetSprite[0] = -1;
				TargetSprite[1] = -1;
			}
			// NO LONGER NEEDED: If the target is valid and the same as the assigned player, handle movement.
		...
		else if ( TargetSprite[0] < 0 )
		{
			...
			{
				#if DEBUG == 1
				LogMessage("Creating target icon...");
				#endif
				
				TargetSprite[0] = AttachParticle(GlobalIndex(false, 1), TARGET_PARTICLE, 32.0);
...
/*	Creates a particle system at the specified entity and returns the effect index, or -1 on error.	*/
stock AttachParticle(entity, String:ParticleName[], Float:offset)
{
	if ( !IsValidEntity(entity) || entity == 0 ) return -1;
	...
	Format(Targetname, sizeof(Targetname), "particle_%d", Particle);
	DispatchKeyValue(Particle, "targetname", Targetname);
	
	new AttachmentIndex = -1;		// Will hold a client index, their model's index or the entity's index.
	decl String:AttachmentName[64];	// Will hold the targetname of the parent.
	
	// I'm hoping m_nModelIndex from the client will point to the entindex of the player's model.
	// We can then parent the sprite to their head bone.
	if ( entity > 0 && entity <= MaxClients && IsClientConnected(entity) )	// If it's a client:
	{
		AttachmentIndex = GetEntPropEnt(entity, Prop_Send, "m_nModelIndex");
		
		if ( AttachmentIndex > MaxClients )	// If getting the model was successful:
		{
			Format(AttachmentName, sizeof(AttachmentName), "clientmodel_%d", AttachmentIndex);
			DispatchKeyValue(AttachmentIndex, "targetname", AttachmentName);
		}
		else	// Otherwise, just use the player entity.
		{
			AttachmentIndex = entity;
			Format(AttachmentName, sizeof(AttachmentName), "client_%d", AttachmentIndex);
			DispatchKeyValue(AttachmentIndex, "targetname", AttachmentName);
		}
	}
	else	// It's a different entity.
	{
		AttachmentIndex = entity;
		Format(AttachmentName, sizeof(AttachmentName), "entity_%d", AttachmentIndex);
		DispatchKeyValue(AttachmentIndex, "targetname", AttachmentName);
	}
	
	DispatchKeyValue(Particle, "parentname", AttachmentName);
	DispatchSpawn(Particle);
	
	new Float:vecPos[3];
	
	// If we set the targetname of the model, SetParentAttachment to bip_head. If we set the targetname to an entity, SetParent.
	// The positioning will happen later when we teleport the particle system.
	if ( entity > 0 && entity <= MaxClients && IsClientConnected(entity) )	// If it's a client:
	{
		if ( AttachmentIndex > MaxClients )	// If it's the model index:
		{
			// Attach the particle to the head bone.
			// This will teleport the particle to the attachment.
			SetVariantString(AttachmentName);
			AcceptEntityInput(Particle, "SetParent");
			SetVariantString("bip_head");
			AcceptEntityInput(Particle, "SetParentAttachment");
			
			// Add on the offset.
			GetEntPropVector(Particle, Prop_Send, "m_vecOrigin", vecPos);
			vecPos[2] += offset;
		...
		else
		{
			...
			GetEntPropVector(AttachmentIndex, Prop_Send, "m_vecOrigin", vecPos);
			vecPos[2] += offset;
			TeleportEntity(Particle, vecPos, NULL_VECTOR, NULL_VECTOR);
		}
	}
	...
	// If the entity is a player and we can get their model, set the targetname name of the model. Otherwise, set the targetname of the entity.
	// Set the parent name of the particle effect accordingly.
	// Spawn the particle.
	// If we set the targetname of the model, SetParentAttachment to bip_head. If we set the targetname to an entity, SetParent.
	// Teleport the particle, activate it, send the "start" input.
}
...
public Action:Panel_Scores(client, args)
{
	...
	{	
		...
		// Red: <n>
		Format(StringBuffer, sizeof(StringBuffer), "%T", "as_this_round", client, TeamScore(false, false, TEAM_RED) );
		DrawPanelText(panel_scores, StringBuffer);
		
		// Blue: <n>
		Format(StringBuffer, sizeof(StringBuffer), "%T", "as_in_total", client, TeamScore(false, true, TEAM_RED) );
		DrawPanelText(panel_scores, StringBuffer);
		...
		// Red: <n>
		Format(StringBuffer, sizeof(StringBuffer), "%T", "as_this_round", client, TeamScore(false, false, TEAM_BLUE) );
		DrawPanelText(panel_scores, StringBuffer);
		
		// Blue: <n>
		Format(StringBuffer, sizeof(StringBuffer), "%T", "as_in_total", client, TeamScore(false, true, TEAM_BLUE) );
		DrawPanelText(panel_scores, StringBuffer);
...
public Action:AdminCommand_Switch(client, args)
{
	...
	// Player type
	Format(StringBuffer, sizeof(StringBuffer), "%T", "as_switch_init_menu_title", client);
	SetPanelTitle(panel_switch_init, StringBuffer);
	
	// Assassin
	Format(StringBuffer, sizeof(StringBuffer), "%T", "as_assassin", client);
	DrawPanelItem(panel_switch_init, StringBuffer);
	
	// Target
	Format(StringBuffer, sizeof(StringBuffer), "%T", "as_target", client);
	DrawPanelItem(panel_switch_init, StringBuffer);
				...
				if ( StrEqual(Buffer, PlayerName) )
				{
					...
					{						
						if ( GetClientTeam(i) == TEAM_RED || GetClientTeam(i) == TEAM_BLUE )
						{
							...
							// If the assassin is now on the same team as the target, re-assign the target to someone on the other team.
							// This should also apply if the assassin and target are the same player.
							if ( GetClientTeam(GlobalIndex(false, 0)) == GetClientTeam(GlobalIndex(false, 1)) )
							{
								switch (GetClientTeam(GlobalIndex(false, 0)))
								{
									case TEAM_RED
									{
										GlobalIndex(true, 1, RandomPlayerFromTeam(TEAM_BLUE));
					...
					else	// Target
					{
						if ( GetClientTeam(i) == TEAM_RED || GetClientTeam(i) == TEAM_BLUE )
						{
							...
							// If the target is now on the same team as the assassin, re-assign the assassin to someone on the other team.
							// This should also apply if the assassin and target are the same player.
							if ( GetClientTeam(GlobalIndex(false, 0)) == GetClientTeam(GlobalIndex(false, 1)) )
							{
								switch (GetClientTeam(GlobalIndex(false, 1)))
