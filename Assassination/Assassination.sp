...
#define PLUGIN_VERSION		"1.2.0.44"
	...
	if ( g_PluginState < STATE_DISABLED )
	{
		...
		if ( AssassinSprite[0] > MaxClients && IsValidEntity(AssassinSprite[0]) )
		{
			...
			if ( GlobalIndex(false, 0) < 1 || GlobalIndex(false, 0) > MaxClients || !IsClientConnected(GlobalIndex(false, 0)) || !IsPlayerAlive(GlobalIndex(false, 0)) || AssassinSprite[1] != GlobalIndex(false, 0) )
			{
				...
				if ( IsValidEntity(AssassinSprite[0]) ) GetEntityClassname(AssassinSprite[0], Classname, sizeof(Classname));
				
				if ( StrEqual(Classname, "info_particle_system", false) )	// Check the name, otherwise we could be killing off some random entity on map start.
				{
					#if DEBUG == 1
					LogMessage("Killing assassin icon...");
					#endif
					
					CreateTimer(3.0, RemoveEnt, EntIndexToEntRef(AssassinSprite[0]));
				}
		...
		else if ( AssassinSprite[0] < 0 )
		{
			...
			if ( GlobalIndex(false, 0) > 0 && GlobalIndex(false, 0) <= MaxClients && IsClientInGame(GlobalIndex(false, 0)) && IsPlayerAlive(GlobalIndex(false, 0)) )
			{
				...
				AssassinSprite[0] = AttachParticle(GlobalIndex(false, 0), ASSASSIN_PARTICLE, SPRITE_OFFSET, true);
		...
		if ( TargetSprite[0] > MaxClients && IsValidEntity(TargetSprite[0]) )
		{
			...
			{
				new String:Classname[64];
				if ( IsValidEntity(TargetSprite[0]) ) GetEntityClassname(TargetSprite[0], Classname, sizeof(Classname));
		...
		else if ( TargetSprite[0] < 0 )
		{
			...
			if ( GlobalIndex(false, 1) > 0 && GlobalIndex(false, 1) <= MaxClients && IsClientInGame(GlobalIndex(false, 1)) && IsPlayerAlive(GlobalIndex(false, 1)) )
			{
				...
				TargetSprite[0] = AttachParticle(GlobalIndex(false, 1), TARGET_PARTICLE, SPRITE_OFFSET, true);
