...
#define PLUGIN_VERSION		"0.0.0.19"	// Note: I don't update this religiously on builds. :P There have been AT LEAST this many builds.
...
public OnMapStart()
{
	...
	n_PreviousAssassinSprite = 0;
	n_PreviousTargetSprite = 0;
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	#if DEBUG == 1
	LogMessage("Player death, damagebits: %d", pd_DamageBits);
	#endif
	...
	else if ( pd_ClientIndex == n_TargetIndex )
	{
		...
		else if ( IsClientConnected(pd_AttackerIndex) && pd_ClientTeam != pd_AttackerTeam )
		{
			...
			else if ( pd_AssisterIndex == n_AssassinIndex && pd_AssisterTeam != pd_ClientTeam && TF2_GetPlayerClass(pd_AssisterIndex) == TF2_GetClass("medic") )	// Or if the assister was an enemy Medic who was the assassin:
			{
				EmitSoundToAll(SND_ASSASSIN_SCORE, _, SNDCHAN_STATIC, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, pd_AttackerIndex, _, NULL_VECTOR, false, 0.0);	// Play assassin killed target
				//LogMessage("Sound played: target killed by assassin.");
			}
...
public Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		new Float:f_healthtoset = float(ph_ClientHealth) + float(ph_ClientDamage / 2);
...
stock SpriteAdmin()
{
	...
	// not alive, set a flag to create the sprite when they respawn.
	
	// If the assassin sprite exists but it's no longer assigned to the assassin:
	if ( n_AssassinSpriteCIndex > 0 && n_AssassinSpriteCIndex <= MaxClients && n_AssassinSpriteCIndex != n_AssassinIndex )
	{
		if ( n_AssassinSpriteIndex > 0 && IsValidEntity(n_AssassinSpriteIndex) )	// If the sprite entity exists
		{
	...
	// If the target sprite exists but it's no longer assigned to the target:
	if ( n_TargetSpriteCIndex > 0 && n_TargetSpriteCIndex <= MaxClients && n_TargetSpriteCIndex != n_TargetIndex )
	{
		if ( n_TargetSpriteIndex > 0 && IsValidEntity(n_TargetSpriteIndex) )	// If the sprite entity exists
		{
			AcceptEntityInput(n_TargetSpriteIndex, "kill");	// Kill the sprite.
			n_TargetSpriteIndex = 0;						// Set its entindex to 0.
			n_TargetSpriteCIndex = 0;						// Set the currently assigned player index to 0.
		}
	}
}
