...
/*
	...
	- Assassin and target sprites have now been replaced by the assassin and target being coloured
		the same colour as their team.
	...
	- The plugin now disables any objective entities on the map, to be used in conjunction with the DM Spawn Manager.
*/
...
#define PLUGIN_VERSION		"1.1.0.45"
...
public Action:Command_Say(client, args)
{
	...
	else if ( StrEqual(text[startidx], "!nfas_help") )
	{
		Panel_Help(client, 0);
...
stock OnAssassinDeath(EventArray[], size, String:Weapon[], String:WeaponLogClassname[], bool:SilentKill)
{
	...
	else if ( IsClientConnected(attacker) && team == GetClientTeam(attacker) )
	{
	...
	else if ( IsClientConnected(attacker) )
	{
...
stock TintPlayers()
{
	...
	for ( new i = 1; i <= MaxClients; i++ )
	{
		if ( IsClientConnected(i) ) SetEntityRenderColor(i, 255, 255, 255, 255);
	}
	
	// If the assassin and target are valid, tint them.
	if ( GlobalIndex[0] != 0 && IsClientConnected(GlobalIndex[0]) )
	{
		switch (GetClientTeam(GlobalIndex[0]))
		{
			case TEAM_RED:
	...
	if ( GlobalIndex[1] != 0 && IsClientConnected(GlobalIndex[1]) )
	{
		...
		*/
		...
		else	// If there are sub-keys:
		{
			do
			{
				...
				#if (DEBUG & DEBUGFLAG_WEAPONMODIFIERS) == DEBUGFLAG_WEAPONMODIFIERS
...
stock Float:ModifyScore(Float:n_Score, id_Weapon, n_CustomKill, i_InflictorIndex)
{
	...
	#if (DEBUG & DEBUGFLAG_MODIFYSCORE) == DEBUGFLAG_MODIFYSCORE
...
stock ItemDefinitionIndex(inflictor, weapon_id)
{
	...
	* to check if the entity is a weapon we could check if the entity name contains "tf_weapon_". */
	...
	#if (DEBUG & DEBUGFLAG_IDINDEX) == DEBUGFLAG_IDINDEX
...
stock DisableObjectives()
{
	...
	{
		#if (DEBUG & DEBUGFLAG_OBJECTIVES) == DEBUGFLAG_OBJECTIVES
