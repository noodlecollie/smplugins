...
public CvarChange( Handle:convar, const String:oldValue[], const String:newValue[] )
{
	...
	if ( convar == cv_Particles ) CheckParticles(GetConVarBool(cv_Particles));
}
...
stock CheckParticles(enable)
{
	...
	{
		if ( enable )	// Create particle effects.
		{
...
public Event_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ( NumPoints > 0 && client > 0 && client <= MaxClients && IsClientInGame(client) )
	{
		...
		new i;
		if ( validSpawns < 1 ) i = GetRandomInt(0, NumPoints-1);	// If there are no valid spawns, spawn anywhere.
		else i = Spawns[GetRandomInt(0, validSpawns-1)];			// Else choose a random valid spawn.
