...
stock Float:CalculateZombieHealthMultiplier()
{
	...
	/*
	// If I were doing this on paper I would write (zMax-zMin) = m(zMaxPl-zMinPl).
	m = (zMax-zMin)/(zMaxPl-zMinPl);
	
	if ( cvDebug & DEBUG_HEALTH == DEBUG_HEALTH ) LogMessage("Gradient %f", m);
	
	// I would then do y=mx+c, or for example zMin = m(zMinPl)+c
	// This means to calculate c we need to do zMin-(m(zMinPl)).
...
stock TintZombie(client)
{
	...
	if ( g_MaxHealth[index] <= 0 )
	{
		SetEntityRenderColor(client, 255, 255, 255, 255);
		return;
	}
	
	// Find out what proportion of the player's original max health they currently have.
	new Float:multiplier = float(g_Health[index]) / float(g_MaxHealth[index]);
	...
	if ( multiplier < zMin ) multiplier = zMin;
	else if ( multiplier > zMax ) multiplier = zMax;
	
	// To calculate each of R, G and B we need to do another y=mx+c.
	// Multiplier is equivalent of x, R/G/B is equivalent of y.
	
	m = (41-255)/(zMax-zMin);					// Calculate the gradient for R.
	c = 255-(zMin * m);							// Calculate the intercept for R.
	new R = RoundFloat((m * multiplier) + c);	// Calculate R.
	
	m = (138-255)/(zMax-zMin);					// Calculate the gradient for G.
	c = 255-(zMin * m);							// Calculate the intercept for G.
	new G = RoundFloat((m * multiplier) + c);	// Calculate G.
	
	m = (30-255)/(zMax-zMin);					// Calculate the gradient for B.
	c = 255-(zMin * m);							// Calculate the intercept for B.
	new B = RoundFloat((m * multiplier) + c);	// Calculate B.
...
{
	//new cvDebug = GetConVarInt(cv_Debug);
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Timer beginning, resetting pack...");
	
	ResetPack(pack);
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Reading userid...");
	new userid = ReadPackCell(pack);
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Declaring vectors...");
	new Float:clientPos[3], Float:clientAng[3];
	
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Reading vectors...");
	...
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Checking userid...");
	if ( userid < 1 ) return;
	
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Getting client index...");
	new client = GetClientOfUserId(userid);
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Checking client index...");
	if ( client < 1 || client > MaxClients || !IsClientInGame(client) ) return;
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Respawning player...");
	TF2_RespawnPlayer(client);
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Teleporting player...");
	TeleportEntity(client, clientPos, clientAng, NULL_VECTOR);
	//if ( cvDebug & DEBUG_CRASHES == DEBUG_CRASHES ) LogMessage("Done.");
}
	...
	for ( new i = 1; i < MaxClients; i++ )
	{
		if ( IsClientInGame(i) )
		{
			...
			if ( GetClientTeam(i) == TEAM_BLUE && IsPlayerAlive(i) && g_Zombie[index] )
			{
				...
				else SetEntProp(i, Prop_Send, "m_iHealth", g_Health[index]);
...
public Action:Timer_CondRefresh(Handle:timer, Handle:pack)
{
	...
	for ( new i = 0; i < MAXPLAYERS; i++ )
	{
		if ( g_StartBoost[i] == true )	// If zombie should be boosted:
		{
			...
			// Client is valid, apply conditions.
